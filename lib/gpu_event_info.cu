...
namespace brainpy_lib {
    namespace {
        ...
        template<typename T>
        inline void nonzero_256(cudaStream_t stream,
                                void **buffers,
                                const char *opaque,
            ...
            const T *events = reinterpret_cast<const T *>(buffers[0]);
            ...
            collect_spike_info<T, 256><<<grid, 256, 0, stream>>>(
                    event_size, events, event_ids, event_num
                    );
