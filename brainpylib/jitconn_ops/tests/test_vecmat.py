...
class Test_vecmat_prob_conn(unittest.TestCase):
  def __init__(self, *args, platform='cpu', **kwargs):
    super(Test_vecmat_prob_conn, self).__init__(*args, **kwargs)
  ...
  def _test_homo(self, shape, transpose, prob, homo_data, seed=None):
    ...
    vector = rng.random(shape[1] if transpose else shape[0]).value

    r1 = bl.jitconn_ops.vecmat_prob_conn_homo_weight(vector,
                                                     homo_data,
                                                     conn_prob=prob,
                                                     shape=shape,
                                                     seed=seed,
    ...
    r2 = bl.jitconn_ops.vecmat_prob_conn_homo_weight(vector,
                                                     homo_data,
                                                     conn_prob=prob,
                                                     shape=shape,
                                                     seed=seed,
  ...
  def _test_homo_vmap(self, shape, transpose, prob, seed=None):
    ...
    rng = bm.random.RandomState()
    events = rng.random((10, shape[1] if transpose else shape[0])).value
    weights = rng.random(10).value

    f1 = jax.vmap(lambda event, data: bl.jitconn_ops.vecmat_prob_conn_homo_weight(
  ...
  def _test_homo_grad(self, shape, transpose, prob, seed=None):
    ...
    rng = bm.random.RandomState()
    events = rng.random(shape[1] if transpose else shape[0]).value < 0.5
    events = events.astype(float)

    f1 = jax.grad(
      lambda event, data: bl.jitconn_ops.vecmat_prob_conn_homo_weight(
  ...
  def _test_uniform(self, shape, transpose, prob, w_low, w_high, seed=None):
    ...
    rng = bm.random.RandomState()
    events = rng.random(shape[1] if transpose else shape[0]).value

    r1 = bl.jitconn_ops.vecmat_prob_conn_uniform_weight(events,
                                                        w_low=w_low,
                                                        w_high=w_high,
                                                        conn_prob=prob,
                                                        shape=shape,
    ...
    r2 = bl.jitconn_ops.vecmat_prob_conn_uniform_weight(events,
                                                        w_low=w_low,
                                                        w_high=w_high,
                                                        conn_prob=prob,
                                                        shape=shape,
  ...
  def _test_uniform_vmap(self, shape, transpose, prob, seed=None):
    ...
    f1 = jax.vmap(lambda e: bl.jitconn_ops.vecmat_prob_conn_uniform_weight(e,
                                                                           w_low=0.,
                                                                           w_high=1.,
                                                                           conn_prob=prob,
                                                                           shape=shape,
  ...
  def _test_uniform_grad(self, shape, transpose, prob, seed=None):
    ...
    f1 = jax.grad(
      lambda e: bl.jitconn_ops.vecmat_prob_conn_uniform_weight(
        e,
        w_low=0.,
        w_high=1.,
        conn_prob=prob,
  ...
  def _test_normal(self, shape, transpose, prob, w_mu, w_sigma, seed=None):
    ...
    r1 = bl.jitconn_ops.vecmat_prob_conn_normal_weight(events,
                                                       w_mu=w_mu,
                                                       w_sigma=w_sigma,
                                                       conn_prob=prob,
                                                       shape=shape,
    ...
    r2 = bl.jitconn_ops.vecmat_prob_conn_normal_weight(events,
                                                       w_mu=w_mu,
                                                       w_sigma=w_sigma,
                                                       conn_prob=prob,
                                                       shape=shape,
  ...
  def _test_normal_vmap(self, shape, transpose, prob, seed=None):
    ...
    f1 = jax.vmap(lambda e: bl.jitconn_ops.vecmat_prob_conn_normal_weight(e,
                                                                          w_mu=0.,
                                                                          w_sigma=1.,
                                                                          conn_prob=prob,
                                                                          shape=shape,
  ...
  def _test_normal_grad(self, shape, transpose, prob, seed=None):
    ...
    rng = bm.random.RandomState()
    events = rng.random(shape[1] if transpose else shape[0]).value < 0.1
    events = events.astype(float)

    f1 = jax.grad(
      lambda e: bl.jitconn_ops.vecmat_prob_conn_normal_weight(
        e,
        w_mu=0.,
        w_sigma=1.,
        conn_prob=prob,
