...
class Test_matvec_prob_conn(unittest.TestCase):
  ...
  def _test_homo(self, shape, transpose, outdim_parallel, prob, homo_data, seed=None):
  ...
  def _test_homo_vmap(self, shape, transpose, outdim_parallel, prob, seed=None):
  ...
  def _test_uniform(self, shape, transpose,outdim_parallel, prob, w_low, w_high, seed=None):
  ...
  def _test_uniform_vmap(self, shape, transpose, outdim_parallel, prob, seed=None):
  ...
  def _test_normal(self, shape, transpose, outdim_parallel, prob, w_mu, w_sigma, seed=None):
  ...
  def _test_normal_vmap(self, shape, transpose, outdim_parallel, prob, seed=None):
          ...
          for shape in self.shapes:
            for prob in [0.01, 0.02, 0.05, 0.1, 0.2, 0.4]:
              for homo_data in [-1., 1.]:
                self._test_homo(shape, transpose,outdim_parallel, prob, homo_data, seed=4366363)
    bm.enable_x64()
    for transpose in [True, False]:
      for outdim_parallel in [True, False]:
        for shape in self.shapes:
      ...
      for outdim_parallel in [True, False]:
        for shape in self.shapes:
          for prob in [0.01, 0.05, 0.1, 0.5]:
            self._test_homo_vmap(shape, transpose, outdim_parallel, prob, seed=4366363)
      ...
      for outdim_parallel in [True, False]:
        for shape in self.shapes:
          for prob in [0.01, 0.05, 0.1, 0.4]:
            self._test_uniform_vmap(shape, transpose, outdim_parallel, prob, seed=4366363)
